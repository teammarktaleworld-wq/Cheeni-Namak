...
const calculateDeliveryCharge = (distance, subtotal) => {
    const dist = parseFloat(String(distance));
    if (!dist || isNaN(dist)) return 0;

    // Free delivery tiers
    if (dist <= 1.5 && subtotal >= 200) return 0;
    if (dist <= 5 && subtotal >= 500) return 0;
...
// ── Coupon definitions ─────────────────────────────────────────────────────────
const COUPONS = {
    CNOFFER10: { discount: 0.10, label: '10% off' },
    CNOFFER15: { discount: 0.15, label: '15% off' },
};
...
const Cart = ({ cartItems, total, isOpen, onClose, removeItem, addToCart, removeFromCart }) => {
    ...
    const RESTAURANT_COORDS = { lat: 26.1270262, lng: 85.3606277 };
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                const distKm = calculateDistance(
                    RESTAURANT_COORDS.lat, RESTAURANT_COORDS.lng, userLat, userLng
                );
                const roadDistanceEst = (distKm * 1.2).toFixed(1);
    ...
    // ── Price calculations ─────────────────────────────────────────────────────
    const discountAmount = appliedCoupon ? Math.round(total * appliedCoupon.discount) : 0;
    const discountedSubtotal = total - discountAmount;
    const deliveryCharge = calculateDeliveryCharge(distance, discountedSubtotal);
    const finalTotal = discountedSubtotal + deliveryCharge;

    // ── Delivery hint text ─────────────────────────────────────────────────────
    const getDeliveryHint = () => {
        if (!distance) return { text: 'Please locate your address to calculate delivery charge.', pulse: false };
        const dist = parseFloat(distance);

        if (dist <= 1.5) {
            if (discountedSubtotal >= 200) return { text: 'YAY! Free Delivery Unlocked! 🎉', pulse: false };
            return { text: `Add ₹${200 - discountedSubtotal} more for FREE delivery (within 1.5 km)! 🚀`, pulse: true };
        }
        if (dist <= 5) {
            if (discountedSubtotal >= 500) return { text: 'YAY! Free Delivery Unlocked! 🎉', pulse: false };
            return { text: `Add ₹${500 - discountedSubtotal} more for FREE delivery (within 5 km)! 🚀`, pulse: true };
        }
        return { text: 'Standard delivery rates apply for > 5 km.', pulse: false };
    };
    ...
    const itemCount = Object.values(cartItems).reduce((sum, item) => sum + item.quantity, 0);
    const showFloatingBar = itemCount > 0 && !isOpen;
    const canCheckout = !locationError && !!distance;
                    ...
                    <div className="bg-white w-full max-w-md rounded-t-2xl sm:rounded-2xl max-h-[90vh] flex flex-col shadow-2xl animate-slide-up">
                        ...
                        <div className="overflow-y-auto p-4 flex-1">
                            ...
                            {Object.values(cartItems).map((item) => (
                                <div key={item.id} className="flex justify-between items-center py-4 border-b border-gray-50 last:border-0 relative group">
                                    <div className="flex-1">
                                        ...
                                        {item.spicyLevel && <p className="text-xs text-orange-600 ml-5">Spicy: {item.spicyLevel}</p>}
                                        <div className="flex items-center gap-3 ml-5 mt-2">
                                            <button onClick={() => removeFromCart(item.id)} className="w-6 h-6 rounded-full border border-gray-300 flex items-center justify-center text-gray-500 hover:bg-gray-100 hover:text-brand-dark transition-colors">-</button>
                                            <span className="font-bold text-brand-dark w-4 text-center">{item.quantity}</span>
                                            <button onClick={() => addToCart(item)} className="w-6 h-6 rounded-full border border-gray-300 flex items-center justify-center text-gray-500 hover:bg-gray-100 hover:text-brand-dark transition-colors">+</button>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <div className="font-bold text-brand-dark">₹{item.price * item.quantity}</div>
                                        <button onClick={() => removeItem(item.id)} className="text-gray-400 hover:text-red-500 transition-colors p-1" aria-label="Remove item">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                                                <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                            </svg>
                                        </button>
                            ...
                            <div className="mt-6 space-y-3">
                                {/* ── Coupon Section ── */}
                                <div className="bg-gray-50 p-3 rounded-lg">
                                    <p className="text-sm font-bold text-brand-dark mb-2">🏷️ Apply Coupon</p>
                                    {appliedCoupon ? (
                                        <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                                            <div>
                                                <span className="text-sm font-bold text-green-700">{appliedCoupon.code}</span>
                                                <span className="text-xs text-green-600 ml-2">({appliedCoupon.label} applied)</span>
                                            </div>
                                            <button
                                        ...
                                        <>
                                            <div className="flex gap-2">
                                                <input
                                                    ...
                                                    className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-red/30 uppercase"
                                                />
                                                <button
                                                    onClick={handleApplyCoupon}
                                                    className="bg-brand-red text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-red-700 transition-colors"
                                                >
                                                    Apply
                                                </button>
                                            </div>
                                ...
                                <div className="bg-gray-50 p-3 rounded-lg">
                                    <div className="flex justify-between items-center mb-1">
                                        <label className="block text-sm font-bold text-brand-dark">Delivery Distance (km)</label>
                                        {!distance && !locationError && (
                                            <span className="text-[10px] font-bold text-white bg-brand-red px-2 py-0.5 rounded-full animate-bounce shadow-md">
                                                👈 Click Locate to proceed
                                            </span>
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            onClick={handleLocateMe}
                                            disabled={isLocating}
                                            className="w-full justify-center bg-blue-600 text-white px-3 py-2 rounded-lg text-sm font-bold hover:bg-blue-700 disabled:bg-blue-300 flex items-center gap-1 transition-all active:scale-95"
                                        >
                                            {isLocating ? (
                                                <svg className="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                    ...
                                    {locationError ? (
                                        ...
                                        <p className={`text-xs mt-2 font-medium ${deliveryHint.pulse ? 'text-brand-red animate-pulse' : 'text-green-600'}`}>
                                            {deliveryHint.text}
                                        </p>
                                    )}
                            ...
                            <div className="mt-6 pt-4 border-t border-dashed border-gray-300 space-y-2">
                                ...
                                <div className="flex justify-between text-sm text-gray-600">
                                    <span>Delivery Charges</span>
                                    <span>{deliveryCharge === 0 && distance ? <span className="text-green-600 font-semibold">FREE</span> : `₹${deliveryCharge}`}</span>
                                </div>
                                <div className="flex justify-between items-center text-xl font-bold text-brand-dark pt-2 border-t border-gray-200">
                                    <span>Total Amount</span>
                                    <span>₹{finalTotal}</span>
                        ...
                        <div className="p-4 bg-gray-50 rounded-b-2xl shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
                            ...
                            <a
                                href={canCheckout ? generateWhatsAppLink() : undefined}
                                target={canCheckout ? '_blank' : undefined}
                                rel="noreferrer"
                                className={`block w-full text-white text-center font-bold py-3.5 rounded-xl shadow-lg transition-colors flex items-center justify-center gap-2
                                    ${canCheckout ? 'bg-[#25D366] hover:bg-[#128C7E]' : 'bg-gray-400 cursor-not-allowed'}`}
                                onClick={(e) => {
                                    if (!canCheckout) {
                                        e.preventDefault();
                                        if (!distance && !locationError) alert("Please use 'Locate' to set your delivery address first.");
                                    }
                                }}
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
