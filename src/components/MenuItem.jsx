...
const MenuItem = ({ item, cart, addToCart, removeFromCart }) => {
  const isDryGravyPair = !!item.dryGravy;

  // Which sub-variant (Dry / Gravy) is currently active. Defaults to Dry.
  const [dryGravySelection, setDryGravySelection] = useState("Dry");
  // Controls the fade/slide transition when switching Dry <-> Gravy.
  const [dryGravyVisible, setDryGravyVisible] = useState(true);

  // The "active" underlying menu item that everything below reads from.
  // For normal items this is just `item` itself, unchanged.
  const activeItem = isDryGravyPair ? item.dryGravy[dryGravySelection] : item;
  ...
  // When switching between Dry and Gravy, reset the Half/Full selection
  // to the new variant's first option (keeps behavior predictable).
  useEffect(() => {
    if (!isDryGravyPair) return;
    const nextVariants = item.dryGravy[dryGravySelection].variants;
    setSelectedVariant(
      nextVariants && nextVariants.length > 0 ? nextVariants[0] : null,
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dryGravySelection]);

  const handleDryGravyChange = (option) => {
    if (option === dryGravySelection) return;
    setDryGravyVisible(false);
    setTimeout(() => {
      setDryGravySelection(option);
      setDryGravyVisible(true);
    }, 150);
  };

  const getQuantity = () => {
    let key = activeItem.id;
    if (selectedVariant) {
      key = `${key}-${selectedVariant.name}`;
    }
    if (activeItem.hasSpicyOption) {
      key = `${key}-${spicyLevel}`;
    }
    return cart[key] ? cart[key].quantity : 0;
  };
  ...
  const handleAdd = () => {
    ...
    if (selectedVariant) {
      ...
      // Single price item
      productToAdd = { ...productToAdd, price: activeItem.price };
    }
    ...
    if (activeItem.hasSpicyOption) {
      ...
      productToAdd = {
        ...
        id: id,
        baseId: activeItem.id,
      };
    } else {
      productToAdd = { ...productToAdd, id: id, baseId: activeItem.id };
    }
  ...
  const fadeStyle = {
    ...
    opacity: dryGravyVisible ? 1 : 0,
    transform: dryGravyVisible ? "translateY(0)" : "translateY(6px)",
  };

  if (activeItem.priceDisplay) {
    return (
      <div className="bg-white rounded-xl shadow-sm p-4 border border-brand-beige/50 hover:shadow-md transition-shadow">
        <div className="flex justify-between items-start">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span
                className={`text-xs px-2 py-0.5 rounded-full border ${activeItem.isVeg ? "border-green-600 text-green-700 bg-green-50" : "border-red-600 text-red-700 bg-red-50"}`}
              >
          ...
          <div className="text-right">
            <span className="block font-bold text-brand-red">
  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 flex flex-col justify-between h-full group transform hover:-translate-y-1">
      ...
      <div className="h-56 overflow-hidden relative">
        ...
        <div className="absolute top-4 right-4">
          <span
            className={`text-[10px] px-3 py-1 rounded-full font-bold uppercase tracking-wider shadow-md ${activeItem.isVeg ? "bg-white text-green-700 border border-green-200" : "bg-white text-red-700 border border-red-200"}`}
          >
            {activeItem.isVeg ? "VEG" : "NON-VEG"}
          </span>
        </div>
          ...
          <div className="flex items-center gap-2 mb-3">
            <div className="flex items-center gap-1 bg-green-100 px-1.5 py-0.5 rounded-md border border-green-200">
              {[...Array(5)].map((_, i) => (
                <svg
                  key={i}
                  className={`w-3 h-3 ${i < Math.floor(activeItem.rating) ? "text-green-700" : "text-green-300"}`}
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
            ...
            <span className="text-xs text-gray-400">
              ({activeItem.reviewCount} reviews)
            </span>
          </div>
          <p
        ...
        <div className="flex-1 space-y-4">
          ...
          isDryGravyPair ? (
            <div className="flex flex-wrap items-center gap-2">
              ...
              {isDryGravyPair && (
                <div
                  className={`flex gap-2 ${activeItem.variants && activeItem.variants.length > 0 ? "ml-2 pl-2 border-l border-gray-200" : ""}`}
                >
                  {["Dry", "Gravy"].map((option) => (
                    <button
                      key={option}
                      onClick={() => handleDryGravyChange(option)}
                      className={`text-xs px-3 py-1.5 rounded-full border font-medium transition-all ${
                        dryGravySelection === option
                          ? "bg-brand-red text-white border-brand-red shadow-md"
                          : "bg-transparent text-gray-500 border-gray-200 hover:border-brand-red hover:text-brand-red"
                      }`}
                    >
